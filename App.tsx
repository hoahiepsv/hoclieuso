...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative pb-24 bg-[#F8FAFC]">
      ...
      {/* Header */}
      <header className="bg-white/95 backdrop-blur-md sticky top-0 z-[60] border-b border-blue-100 px-4 md:px-12 py-4 flex flex-wrap justify-between items-center gap-4 shadow-sm">
        <div className="flex items-center gap-4 group cursor-pointer" onClick={() => handleLogout()}>
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-black text-blue-900 uppercase tracking-tighter leading-none">{APP_NAME}</h1>
            <p className="text-[8px] md:text-[9px] text-blue-400 font-bold tracking-widest uppercase mt-1">AI-POWERED PLATFORM</p>
          </div>
        </div>

        {/* API Key Management Area */}
        <div className="flex-grow max-w-xl mx-4 hidden lg:flex items-center gap-2">
          <div className="relative flex-grow">
            <input
              ...
              className={`w-full text-xs font-bold py-2.5 px-4 rounded-xl border-2 transition-all outline-none ${
                isEditingKey 
                  ? "border-blue-400 bg-white text-blue-900 shadow-md" 
                  : "border-transparent bg-blue-50/50 text-blue-300"
              }`}
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              className="px-4 py-2.5 bg-white border-2 border-blue-100 text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-50 transition-all shadow-sm"
            >
              HƯỚNG DẪN TẠO MÃ GEMINI API
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          {appState.currentUser && (
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <button onClick={handleLogout} className="bg-blue-50 text-blue-600 p-2.5 md:p-3 rounded-xl hover:bg-blue-600 hover:text-white transition-all shadow-sm border border-blue-100">
      ...
      {showGuide && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-blue-900/40 backdrop-blur-md animate-in">
          <div className="bg-white rounded-[40px] shadow-2xl p-10 max-w-2xl w-full border border-blue-100 relative overflow-hidden max-h-[90vh] overflow-y-auto">
            <button onClick={() => setShowGuide(false)} className="absolute top-6 right-6 text-gray-300 hover:text-red-500 transition-colors">
               <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
            <div className="mb-8">
               <h3 className="text-2xl font-black text-blue-900 uppercase tracking-tighter">HƯỚNG DẪN TẠO MÃ GEMINI API</h3>
               <p className="text-[10px] text-blue-400 font-bold uppercase tracking-widest mt-2">Để kích hoạt tính năng thông minh của AI</p>
            </div>
            
            <div className="space-y-4 text-sm font-medium text-slate-700">
               <div className="flex gap-4 items-start p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black shrink-0">B1</span>
                  <p className="pt-1">Truy cập link: <a href="https://aistudio.google.com/app/api-keys" target="_blank" rel="noopener noreferrer" className="text-blue-600 font-black underline">https://aistudio.google.com/app/api-keys</a></p>
               </div>
               <div className="flex gap-4 items-start p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black shrink-0">B2</span>
                  <p className="pt-1">Bấm <strong>Create APIKey</strong> (phía trên bên phải)</p>
               </div>
               <div className="flex gap-4 items-start p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black shrink-0">B3</span>
               ...
               <div className="flex gap-4 items-start p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black shrink-0">B4</span>
                  <p className="pt-1">Tại ô <strong>Choose an imported project</strong>: chọn <strong>Default Gemini Project</strong></p>
               </div>
               <div className="flex gap-4 items-start p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black shrink-0">B5</span>
                  <p className="pt-1">Bấm <strong>Create key</strong></p>
               </div>
               <div className="flex gap-4 items-start p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black shrink-0">B6</span>
                  <div>
               ...
               <div className="flex gap-4 items-start p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black shrink-0">B7</span>
                  <p className="pt-1">Mở phần mềm ra, bấm <strong>CHỈNH SỬA</strong> và dán mã API này vào ô .</p>
               </div>
               <div className="flex gap-4 items-start p-4 bg-blue-50 rounded-2xl border border-blue-100">
                  <span className="w-8 h-8 bg-blue-600 text-white rounded-lg flex items-center justify-center font-black shrink-0">B8</span>
            ...
            <button 
              ...
              className="w-full mt-8 bg-blue-600 text-white py-4 rounded-2xl font-black uppercase tracking-widest hover:bg-blue-700 shadow-xl transition-all"
            >
              ĐÃ HIỂU
            </button>
          </div>
